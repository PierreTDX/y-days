...
export function QuizCard({
    ...
    return (
        <Card className={`p-0 gap-0 border-2 ${gameResult !== null ? (gameResult ? 'border-green-500' : 'border-red-500') : 'border-border'}`}>
            <div className={`relative flex items-center justify-center text-muted-foreground overflow-hidden rounded-t-xl ${mediaClassName || 'aspect-video bg-muted max-h-[320px]'}`}>
                {media}

                {gameResult !== null && (
                    <div className="absolute top-3 right-3 flex items-center gap-2 z-20">
                        <span className={`text-sm font-medium drop-shadow-sm ${gameResult ? 'text-green-500' : 'text-red-500'}`}>
