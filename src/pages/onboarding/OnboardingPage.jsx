...
const LIST_ITEMS = [
  { icon: '⚙️', text: "L'Introduction aux outils d'IA : outils généraux et outils pour la génération de ressources pédagogiques." },
  { icon: '✏️', text: "La méthode pour créer une instruction (prompt) efficace et optimisée pour générer un cours pédagogique pertinent avec l'IA." },
  { icon: '💡', text: "Quelques cas concrets sur divers utilisation des outils d'IA." },
  { icon: '✅', text: "Comment s'assurer que le contenu généré est qualitatif, et adapté vos usages." },
]
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div className="glass" style={{ padding: 24 }}>
        <div style={{ fontSize: 48, fontWeight: 700, color: "#4f46e5", letterSpacing: "-1px" }}>
          0{hours} h 00
        </div>
      ...
      <div className="glass" style={{ padding: 24 }}>
        <div style={{ fontSize: 48, fontWeight: 700, color: "#06b6d4", letterSpacing: "-1px" }}>
          0{reforms}
        </div>
        <p style={{ marginTop: 10, color: "#d1d5db", lineHeight: 1.65, marginBottom: 0 }}>
      ...
      <div className="glass" style={{ padding: 32, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {STEPS.map((_, i) => (
              <div key={i} style={{ height: 8, width: i === step ? 24 : 8, borderRadius: 4, background: i === step ? "#4f46e5" : "rgba(255,255,255,0.2)", transition: "background 0.3s, width 0.3s" }} />
            ))}
          </div>
        </div>
